...
const CARS_API_URL = 'https://multipost-public.app.infinit.cc/api/public/inventory/profiles/0069c477-17e4-476e-a485-1367deafe125';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const pageTitle = `${titleParts.join(' ')} - ${formattedPrice} | MCM Cars`;

    // Build description parts
    const descParts = [vehicle.brand, vehicle.model, vehicle.year].filter(Boolean);
    const pageDescription = `${descParts.join(' ')} - ${mileageText}, ${vehicle.fuel}, ${vehicle.transmission}. Vehículo de alta gama disponible en MCM Cars Madrid.`;

    const pageUrl = `https://www.montecarmelomadridcars.com/buy/${id}`;
    let carImage = vehicle.images && vehicle.images.length > 0 ? vehicle.images[0] : 'https://www.montecarmelomadridcars.com/placeholder.svg';
    ...
    if (carImage && carImage.includes('alxproduction.blob.core.windows.net')) {
      ...
      carImage = `https://www.montecarmelomadridcars.com/_vercel/image?url=${encodedUrl}&w=1600&q=80`;
    }
...
<html lang="es">
  <head>
    <meta charset="UTF-8" />
    <link rel="icon" type="image/png" href="https://www.montecarmelomadridcars.com/favicon.png" />
    <link rel="shortcut icon" type="image/png" href="https://www.montecarmelomadridcars.com/favicon.png" />
    ...
    <meta property="og:site_name" content="MCM Cars" />
